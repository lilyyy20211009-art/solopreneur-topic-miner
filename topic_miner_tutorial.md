...
## 🛠️ 第二章：進階挑戰 —— 如何從零寫出一個爆款選題 Agent？

如果你不滿足於「抄作業」，想自己親手用 Trae 從零寫出一個類似 `solopreneur-topic-miner` 這樣強大的專屬 AI 助手，請跟著以下詳細步驟與具體指令進行操作。

### Step 1: 召喚 Skill 創建器，建立基礎框架
首先，我們需要在 Trae 中建立一個新的 Skill 項目。
**你的操作（在 Trae 對話框中輸入）**：
> 「我想創建一個名為 `my-topic-miner` 的 Skill。這個 Skill 的作用是作為一個資深的爆款選題主編，幫我每天自動抓取微博、知乎等平台熱搜，並根據我的賽道（如 AI 效率工具）打分，最後給出 6 個爆款選題。請幫我初始化這個 Skill。」

*Trae 收到指令後，會自動調用 `skill-creator` 工具，在你的專案中建立 `.trae/skills/my-topic-miner/SKILL.md` 文件。*

### Step 2: 編寫底層爬蟲腳本（賦予 Agent 抓取數據的能力）
大模型本身無法實時連網看熱搜，我們需要讓 Trae 幫我們寫一段 Python 代碼來充當「情報員」。
**你的操作（在 Trae 對話框中輸入）**：
> 「為了讓 `my-topic-miner` 能夠獲取真實數據，請幫我在 `.trae/skills/my-topic-miner/` 目錄下寫一個 Python 腳本，命名為 `fetch_trends.py`。
> 具體要求：
> 1. 使用 `requests` 庫。
> 2. 調用免費的開源 API（如 https://60s.viki.moe/v2/weibo 和 https://60s.viki.moe/v2/zhihu ）。
> 3. 將抓取到的前 10 條熱搜標題打印出來。
> 4. 支持通過命令行參數 `--niche` 傳入我的賽道名稱，並根據賽道關鍵詞進行簡單過濾。」

*Trae 會自動生成代碼並保存為 `fetch_trends.py`。*

### Step 3: 編寫核心 Prompt（注入主編靈魂與工作流）
這一步最關鍵。你需要打開剛才生成的 `SKILL.md` 文件，把裡面默認的提示詞，替換成我們精心設計的「主編工作流」。
**你的操作（直接將以下內容複製替換到 `SKILL.md` 中）**：

```markdown
---
name: "my-topic-miner"
description: "自動抓取全網熱點，結合用戶賽道進行三維打分，輸出 Top 6 爆款選題及大綱。"
---

# 爆款選題挖掘 Agent

你是一個資深的「爆款選題主編 Agent」。你的任務是根據用戶提供的【創作賽道】，利用網絡搜索工具獲取實時熱點，並為用戶自動挖掘出**最值得動筆的 6 個爆款選題**。

## 🎯 啟動與記憶
當用戶召喚本 Skill 時：
1. 詢問用戶的**創作賽道/領域**（例如：AI 效率工具、自媒體運營等）。
2. 詢問用戶的**目標受眾定位**。

## ⚙️ 核心工作流
收到用戶輸入後，你必須嚴格按照以下步驟執行：

### Step 1: 獲取真實熱點數據
**【強制動作】**：你必須使用終端工具運行命令 `python3 .trae/skills/my-topic-miner/fetch_trends.py --niche "[用戶賽道]"`。
**【紅線】**：絕對禁止憑空捏造熱點！必須基於腳本返回的真實數據。

### Step 2: 決策層 —— 三維漏斗打分
閱讀腳本返回的數據，對每個熱點進行打分（1-10分）：
1. **趨勢分**：話題目前討論度高不高？
2. **競爭分**：普通人切入是否還有流量紅利？（競爭越小，得分越高）
3. **匹配分**：與用戶的【創作賽道】契合度有多高？
*動作*：計算總分，只保留排名前 6 的話題。

### Step 3: 輸出格式
對於入選的 Top 6，請使用以下 Markdown 格式輸出：
### 🏆 選題一：[一句話概括話題]
*   **🔥 熱點來源**：...
*   **📊 數據得分**：趨勢 X分 | 競爭 X分 | 匹配 X分
*   **💡 差異化切入點**：給出 2 個別人沒寫過的新奇視角。
*   **✍️ 備選標題**：提供 3 個極具網感的標題。
*   **📝 內容大綱**：引言(Hook) -> 核心觀點(Body) -> 行動呼籲(CTA)
```

### Step 4: 測試與迭代防幻覺
代碼和 Prompt 都寫好後，開始測試。
**你的操作（在 Trae 對話框中輸入）**：
> `啟動 my-topic-miner`

**觀察與優化**：
*   **如果 AI 開始瞎編新聞**：你就在對話框裡罵它：「你剛才捏造了熱點！去修改 `SKILL.md`，加上『絕對禁止猜測，必須等待 Python 腳本執行完畢後再回答』的強制指令！」
*   **如果 AI 忘記執行腳本**：你告訴 Trae：「你沒有執行 `fetch_trends.py`，請優化 `SKILL.md` 的 Step 1，明確要求必須使用 `RunCommand` 工具執行代碼。」

經過這 4 個步驟的「編寫指令 -> 寫代碼 -> 寫 Prompt -> 測試修復」，一個完全屬於你自己的自動化選題 Agent 就誕生了！
